...
def start_teams_bot(start_audio_record_event, meeting_url, bot_name):
    ...
    start_audio_record_event.set() # Set the audio record event
    # Run the javascript code, has an implict infinite loop
    print("MicrosoftTeams Tasks are Finiky: You will need to manually STOP this task, when the meeting has ended")
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    print(driver.execute_script("return localStorage.getItem('audioLooping');"))
                    if meeting_is_ongoing is False:
                        print("Hurray, the Meeting has ended.")
                        stop_code_execution(driver) # allows all threads to end
                    elif meeting_is_ongoing is None and (not 'teams.microsoft.com' in BOT.meeting_url) :
                        print("You may need to manually STOP this task, if the meeting may has ended")
                except:
                    pass
            except Exception as e:
                traceback.print_exc()
