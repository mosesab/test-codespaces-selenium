...
# Pre-patch ChromeDriver with undetected-chromedriver
RUN echo "import undetected_chromedriver as uc; \
    patcher = uc.Patcher(executable_path='/var/local/chromedriver-linux64/chromedriver', \
    force=True); \
    patcher.patch_exe()" > patch_chromedriver.py && \
    python3 patch_chromedriver.py

# Clean up stale X server lock file
RUN rm -f /tmp/.X0-lock

#Start a Xvfb process in the background
RUN Xvfb :1 -screen :1 1280x1024x16 &
